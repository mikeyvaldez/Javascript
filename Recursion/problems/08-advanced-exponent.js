...
function advancedExponent(baseNum, power){
    // if the power is 0 return 1
    if(power === 0) return 1;
    // if the power is 1 return the baseNum
    if(power === 1) return baseNum;
    // if the power is 2 return 2 times the baseNum
    if(power === 2) return 2 * baseNum;

    // if the power is even create a recursive variable
    // to handle dividing the power by 2
    // in the same block you rturn the halved exponent
    // multiplied by itself
    if(power % 2 === 0){
        const halfExponent = advancedExponent(baseNum, power/2);
        return halfExponent * halfExponent;
    } else {
        // otherwise, create a variable to handle
        // the power subtracting one but also being divided by 2
        // in the same block you you multiply the baseNum to the
        // halved exponent then again to the halved exponent
        const halfExponent = advancedExponent(baseNum, (power - 1) / 2);
        return baseNum * halfExponent * halfExponent;
    }
